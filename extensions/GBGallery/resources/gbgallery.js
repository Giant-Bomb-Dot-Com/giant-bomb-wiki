( function () {
	var items = Array.from( document.querySelectorAll( '.gb-gallery-item[data-thumb]' ) );
	if ( !items.length ) {
		return;
	}
	...
	function open( index ) {
		if ( !overlay ) {
			...
			overlay.querySelector( '.gb-lightbox-close' ).addEventListener( 'click', close );
			overlay.querySelector( '.gb-lightbox-prev' ).addEventListener( 'click', function () {
				show( ( currentIndex - 1 + items.length ) % items.length );
			} );
			overlay.querySelector( '.gb-lightbox-next' ).addEventListener( 'click', function () {
