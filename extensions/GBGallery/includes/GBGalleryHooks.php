...
class GBGalleryHooks implements ParserFirstCallInitHook {
    ...
    public static function renderGallery(
        ...
        if ( $config === null ) {
            ...
            $parser->getOutput()->setExtensionData( 'gb-gallery-config', $config );
        }
        ...
        foreach ( $lines as $line ) {
            ...
            $thumbUrl = $config['baseUrl'] . '/' . $config['thumbSize'] . '/' . $imagePath;
            $fullUrl  = $config['baseUrl'] . '/' . $config['fullSize'] . '/' . $imagePath;
