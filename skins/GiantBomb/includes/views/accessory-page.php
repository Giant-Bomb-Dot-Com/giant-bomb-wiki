...
$accessoryData = [
	'name' => str_replace( 'Accessories/', '', str_replace( '_', ' ', $pageTitle ) ),
...
try {
	...
	if ( $content ) {
		...
		if ( $wikitext !== '' ) {
			$descCacheKey = $latestRevisionId > 0 ? $wanCache->makeKey( 'giantbomb-accessory-desc', $latestRevisionId ) : null;
			...
			if ( !is_array( $descData ) ) {
				...
				try {
					...
					error_log( 'Failed to parse accessory wikitext: ' . $e->getMessage() );
					$descHtml = $wikitext;
				}
			...
			$accessoryData['description'] = $descData['html'] ?? '';
		}

		$accessoryData['name'] = gbGetFieldValue( $infoboxFields, [ 'name' ] ) ?: $accessoryData['name'];
		$accessoryData['deck'] = gbGetFieldValue( $infoboxFields, [ 'deck' ] );
		$accessoryData['image'] = gbGetFieldValue( $infoboxFields, [ 'image', 'infoboximage' ] );
		$accessoryData['guid'] = gbGetFieldValue( $infoboxFields, [ 'guid', 'id' ] );
		$accessoryData['manufacturer'] = gbGetFieldValue( $infoboxFields, [ 'manufacturer', 'manufacturers' ] );
		$accessoryData['developers'] = gbGetFieldValue( $infoboxFields, [ 'developer', 'developers' ] );
		$accessoryData['releaseDate'] = gbGetFieldValue( $infoboxFields, [ 'release', 'releasedate', 'release date' ] );
		$accessoryData['connectivity'] = gbGetFieldValue( $infoboxFields, [ 'connectivity', 'connection' ] );
		$accessoryData['price'] = gbGetFieldValue( $infoboxFields, [ 'price', 'msrp' ] );
		...
		if ( $rawAliases !== '' ) {
			...
			$accessoryData['aliases'] = array_values( $aliases );
			$accessoryData['aliasesDisplay'] = implode( ', ', $accessoryData['aliases'] );
		}

		$rawPlatforms = gbGetFieldValue( $infoboxFields, [ 'platforms', 'platform' ] );
		if ( $rawPlatforms !== '' ) {
			$platforms = array_filter(
				array_map(
					static function ( $item ) {
						...
						$item = preg_replace( '#^(Platforms|Games)/#', '', $item );
						return str_replace( '_', ' ', $item );
					},
					explode( ',', $rawPlatforms )
				)
			);
			$accessoryData['platforms'] = array_values( $platforms );
		}
			...
			$items = array_filter(
				array_map(
					static function ( $item ) {
						...
						$item = preg_replace( '#^(Games|Franchises|Companies|Objects|Platforms)/#', '', $item );
						return str_replace( '_', ' ', $item );
					},
					explode( ',', $rawList )
				)
			);
			$accessoryData['relations'][$key] = array_values( $items );
		}

		$resolvedTemplateImage = gbResolveWikiImageUrl( $accessoryData['image'] );
		$accessoryData['image'] = $resolvedTemplateImage ?? '';

		if ( isset( $legacyImageData ) && is_array( $legacyImageData ) ) {
			if ( $accessoryData['image'] === '' && isset( $legacyImageData['infobox'] ) ) {
				$infoboxUrl = gbBuildLegacyImageUrl(
					$legacyImageData['infobox'],
					[ 'scale_super', 'screen_kubrick', 'screen_medium', 'scale_large', 'scale_medium' ]
				);
				if ( $infoboxUrl !== null ) {
					$accessoryData['image'] = $infoboxUrl;
				}
			}
			if ( isset( $legacyImageData['background'] ) ) {
				...
				if ( $backgroundUrl !== null ) {
					$accessoryData['backgroundImage'] = $backgroundUrl;
				}
			}
			if ( $accessoryData['image'] === '' && $accessoryData['backgroundImage'] !== '' ) {
				$accessoryData['image'] = $accessoryData['backgroundImage'];
			}
		}
	}
} catch ( \Throwable $e ) {
...
if ( $accessoryData['manufacturer'] !== '' ) {
	$stats[] = [
		'label' => 'Manufacturer',
		'value' => $accessoryData['manufacturer'],
	];
}
if ( $accessoryData['developers'] !== '' ) {
	$stats[] = [
		'label' => 'Developer',
		'value' => $accessoryData['developers'],
	];
}
if ( $accessoryData['platforms'] !== [] ) {
	$stats[] = [
		'label' => 'Platforms',
		'value' => implode( ', ', $accessoryData['platforms'] ),
	];
}
if ( $accessoryData['releaseDate'] !== '' ) {
	$stats[] = [
		'label' => 'Release date',
		'value' => $accessoryData['releaseDate'],
	];
}
if ( $accessoryData['connectivity'] !== '' ) {
	$stats[] = [
		'label' => 'Connectivity',
		'value' => $accessoryData['connectivity'],
	];
}
...
$metaTitle = $accessoryData['name'] !== ''
	...
	: 'Giant Bomb Wiki';
$metaDescription = gbSanitizeMetaText( $accessoryData['deck'] ?? '' );
if ( $metaDescription === '' ) {
	$metaDescription = gbSanitizeMetaText( $accessoryData['description'] ?? '' );
}
if ( $metaDescription === '' && $accessoryData['name'] !== '' ) {
	$metaDescription = 'Learn about the ' . $accessoryData['name'] . ' accessory on the Giant Bomb wiki.';
}
$metaImage = $accessoryData['image'] !== '' ? $accessoryData['image'] : ( $accessoryData['backgroundImage'] !== '' ? $accessoryData['backgroundImage'] : null );
$canonicalUrl = rtrim( GB_PUBLIC_WIKI_HOST, '/' ) . $accessoryData['url'];
...
if ( $metaImage ) {
	...
	$twitterTags['twitter:image:alt'] = $accessoryData['name'] !== '' ? $accessoryData['name'] . ' accessory image' : 'Giant Bomb accessory cover art';
}
...
if ( $accessoryData['manufacturer'] !== '' ) {
	$schema['manufacturer'] = [
		'@type' => 'Organization',
		'name' => $accessoryData['manufacturer'],
	];
}
if ( $accessoryData['developers'] !== '' ) {
	$schema['brand'] = [
		'@type' => 'Organization',
		'name' => $accessoryData['developers'],
	];
}
if ( $accessoryData['releaseDate'] !== '' ) {
