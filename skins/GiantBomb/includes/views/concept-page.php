...
$conceptData = [
	'name' => str_replace( 'Concepts/', '', str_replace( '_', ' ', $pageTitle ) ),
...
try {
	...
	if ( $content ) {
		...
		if ( $wikitext !== '' ) {
			$descCacheKey = $latestRevisionId > 0 ? $wanCache->makeKey( 'giantbomb-concept-desc', $latestRevisionId ) : null;
			...
			if ( !is_array( $descData ) ) {
				...
				try {
					...
					error_log( 'Failed to parse concept wikitext: ' . $e->getMessage() );
					$descHtml = $wikitext;
				}
			...
			$conceptData['description'] = $descData['html'] ?? '';
		}
		...
		if ( preg_match( '/\| Aliases=([^\n]+)/', $text, $matches ) ) {
			...
			$conceptData['aliases'] = array_values( $aliases );
				...
				$items = array_filter(
					array_map(
						static function ( $item ) {
							...
							$item = preg_replace( '#^(Concepts|Franchises|Games|Characters|Locations|Objects|People)/#', '', $item );
							return str_replace( '_', ' ', $item );
						},
						explode( ',', $matches[1] )
					)
				);
				$conceptData['related'][$key] = array_values( $items );
			}
		}

		$resolvedTemplateImage = gbResolveWikiImageUrl( $conceptData['image'] );
		$conceptData['image'] = $resolvedTemplateImage ?? '';

		if ( isset( $legacyImageData ) && is_array( $legacyImageData ) ) {
			if ( $conceptData['image'] === '' && isset( $legacyImageData['infobox'] ) ) {
				$infoboxUrl = gbBuildLegacyImageUrl(
					$legacyImageData['infobox'],
					[ 'scale_super', 'screen_kubrick', 'screen_medium', 'scale_large', 'scale_medium' ]
				);
				if ( $infoboxUrl !== null ) {
					$conceptData['image'] = $infoboxUrl;
				}
			}
			if ( isset( $legacyImageData['background'] ) ) {
				...
				if ( $backgroundUrl !== null ) {
					$conceptData['backgroundImage'] = $backgroundUrl;
				}
			}
			if ( $conceptData['image'] === '' && $conceptData['backgroundImage'] !== '' ) {
				$conceptData['image'] = $conceptData['backgroundImage'];
			}
		}
	}
} catch ( \Throwable $e ) {
...
$metaTitle = $conceptData['name'] !== ''
	...
	: 'Giant Bomb Wiki';
$metaDescription = gbSanitizeMetaText( $conceptData['deck'] ?? '' );
if ( $metaDescription === '' ) {
	$metaDescription = gbSanitizeMetaText( $conceptData['description'] ?? '' );
}
if ( $metaDescription === '' && $conceptData['name'] !== '' ) {
	$metaDescription = 'Dive into the Giant Bomb wiki concept page for ' . $conceptData['name'] . '.';
}
$metaImage = $conceptData['image'] !== '' ? $conceptData['image'] : ( $conceptData['backgroundImage'] !== '' ? $conceptData['backgroundImage'] : null );
$canonicalUrl = rtrim( GB_PUBLIC_WIKI_HOST, '/' ) . $conceptData['url'];
...
if ( $metaImage ) {
	...
	$twitterTags['twitter:image:alt'] = $conceptData['name'] !== '' ? $conceptData['name'] . ' concept art' : 'Giant Bomb concept cover art';
}
