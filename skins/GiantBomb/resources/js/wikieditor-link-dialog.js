 ...
 * Updated to prevent stacking multiple different prefixes.
 */
$(document).on('dialogopen', function(event) {
    ...
    allPrefixes = config.map(function(item) {
    ...
    $.each(config, function(i, btn) {
        $('<button>')
            ...
            .css({'cursor': 'pointer', 'margin-right': '5px', 'padding': '1px 6px', 'font-size': '0.85em'})
            .on('click', function(e) {
                e.preventDefault();
                ...
                var sanitized = baseTitle.replace(/[\s!\"#$%&'()*+,\-./:;<=>?@\[\\\]^`{|}~]+/g, '_');
