<template>
  <main class="releases-main">
    ...
    <div v-else-if="weekGroups.length > 0">
      <div v-for="weekGroup in weekGroups" :key="weekGroup.label" class="week-group">
        <h2 class="week-label">{{ weekGroup.label }}</h2>
        <div class="releases-grid">
          <div v-for="(release, index) in weekGroup.releases" :key="index" class="release-card">
            <a :href="release.url" class="release-card-link">
              ...
              <div v-else class="release-image release-image-placeholder">
                <img src="https://www.giantbomb.com/a/uploads/original/11/110673/3026329-gb_default-16_9.png" alt="Giant Bomb Default Image" loading="lazy">
              </div>
              
              <div class="release-info">
                <h3 class="release-title">{{ release.title }}</h3>
                <div class="release-date">
                  <span v-if="release.releaseDateFormatted">{{ release.releaseDateFormatted }}</span>
                  <img 
                    ...
                    loading="lazy"
                  />
                  <span v-else-if="release.region" class="region-text"> ({{ release.region }})</span>
                </div>
                
                <div v-if="release.platforms && release.platforms.length > 0" class="release-platforms">
                  <span 
